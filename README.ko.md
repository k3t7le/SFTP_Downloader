...
- **마지막 처리 시각 기억** — `TempWorkspaceRoot/state/job-state.json`에 잡·원격 폴더별 최근 성공 시각과 그 시각에 처리한 파일명을 함께 저장합니다. 저장된 시각보다 새로운 파일만 받고, 같은 시각의 기존 파일명은 건너뛰며(1초 안전 여유 포함) 누락을 방지합니다.
